...
$_SESSION['uID']=1;
$_SESSION['cID']=$rID=$ro->build_request_variable('c');
$rID=$ro->build_request_variable('r');
if (!empty($rID)) {
	$array['n']=$ro->get_room_name($rID);
	$b=$array['n']['b'];
	$r=$array['n']['r'];
	$array['r']=$ro->get_room_coordinates($rID);
}
/*
echo '<pre>';
print_r($array);
...
<head>
    ...
    <script>
		<?php if (!empty($b)) echo 'var building = "'.$b.'";'."\n"; ?>
		<?php if (!empty($r)) echo 'var room_number = "'.$r.'";'."\n"; ?>
		...
		<div id="left_side">
			...
			<div id="classroom_mapping">
				<div id="section_input">
					<table>
					<caption id="section_id" style="font-size: small; padding-top: 7px">Section</caption>
					<tr>
						...
						<td rowspan="3" id="section_events" width="130" nowrap></td>
					</tr>
					<tr>
		...
		<div id="right_side">
			<div class="dropdown">
				...
				<ul class="dropdown-menu">
					<?php foreach ($array['a']['Instructor Activity'] as $a => $b) echo '<li><a title="'.$b.'" id="'.$a.'" onclick="logData('."'".$a."'".')">'.$b.'</a></li>'."\n"; ?>
				</ul>
			</div>
			<div class="dropdown">
			    <!-- trigger button -->
			    ...
			    <!-- dropdown menu -->
			    <ul class="dropdown-menu">
					<?php foreach ($array['a']['Student Activity'] as $a => $b) echo '<li><a title="'.$b.'" id="'.$a.'" onclick="logData('."'".$a."'".')">'.$b.'</a></li>'."\n"; ?>
			    </ul>
			</div>
